...
import (
	"fmt"
	_ "github.com/CsBoBoNice/Csfile"
	_ "io/ioutil"
	"os"
	"path/filepath"
	"strings"
...
func WalkDirFile(dirPth, suffix string) (files []string, dirs []string, err error) {
...
func Mkdirs(name []string, ShareDir string, DirHead string) (err error) {
	for index, value := range name {
		//fmt.Println("Index = ", index, "Value = ", value)
		if index != 0 {
			dir := GetShareDir(value, ShareDir, DirHead)
			err = MakeDir(dir)
			if err != nil {
				return
			}
